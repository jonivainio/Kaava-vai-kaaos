...
describe("v5 K60 full source-to-runtime and artwork inventory", () => {
  it("records all 248 IDs, each source branch and actual art paths without assuming a whole campaign from text import", () => {
    const rows = content.map(item => {
      ...
      const group = rule ? Object.entries(groups).find(([, rules]) => rules.includes(rule))![0] : null;
      ...
      if (rule) {
        ...
        for (const mode of ["hybrid", "wind", "solar"] as const) for (let seed = 0; seed < 20; seed++) {
          ...
          art.add(currentDecision(game)!.art);
        }
      }
      ...
      return { id: item.id, kind: item.kind, stage: item.stage,
        ...
        sourceBranches, branchTests: sourceBranches.length ? "tests/v5-results.test.ts; reports/v5/result-branch-tests.json" : "No named source branch; see K01–K60 validation matrix",
        artKeys: [...art], artFiles: [...art].map(key => `public/art/${key}.svg`), presentation: rule ? "SwipeCard:decision" : special,
        runtimeModes: ["hybrid:subject to the registered region/component/case eligibility", "wind/solar:closed-mode-preview; no separate menu release"],
        status: "INTEGRATED_TESTED", note: "Binding/art evidence here; behavioral evidence in result-branch-tests.json and QA_V5.md. A solar-base suffix is an alias name, not a blanket hybrid exclusion. Separate wind/solar menu modes remain intentionally closed." };
    });
    expect(rows).toHaveLength(248); expect(rows.reduce((sum, row) => sum + row.sourceBranches.length, 0)).toBe(143);
    mkdirSync("reports/v5", { recursive: true });
    writeFileSync("reports/v5/content-coverage.json", JSON.stringify({ contentVersion: CONTENT_VERSION, rows }, null, 2) + "\n");
  });
});
