...
Ensisijainen pelaajasisältö: `docs/source-v5/KAAVA_VAI_KAAOS_CODEX_v5/KAAVA_VAI_KAAOS_TARKISTUSDOKUMENTTI_v5.md`, SHA-256 `03ae272389667c41412e8fef7a86f091db147aa72fbe6907a54b79661d3003f2`. `tools/import_v5.py --check` tarkistaa kaikki 248 ID:tä, 318 valintaa ja 143 tuloshaaraa. Lähdettä ei muokata. Käyttäjän 10.9. pyytämät tarinatiivistelmät, tulosten yhdistely ja harkitut reaktiot ovat erillisessä `narration.ts`-esityskerroksessa. Päätösten otsikot, kysymykset ja valinnat säilyvät lähteen mukaisina. Sallitut muuttujat ja lukusanaan liittyvä kieliopillinen taivutus täytetään näytössä.
...
`GameV5` on serialisoitava tietue. Tunnisteet: tallennus `swipe-v5-1`, sisältö `v5-fi-03ae27238966`, säännöt `v5-rules-3`. Se sisältää lähtötilan, nykyiset fyysiset paikat, piilotetun maailmaprofiilin, neljä kehitysvaihetta, työkalenterin, tapaukset, tulosjonon, sopimukset, BESSin, lupatilat, kustannukset, laatuhavainnot, päätöshistorian ja käyttöliittymälle tarkoitetut kohtaukset. `assetChanges` säilyttää kunkin HUD-luvun viimeisimmän todellisen ennen/jälkeen-arvon ja muutosrevision. Preview ei kirjoita tätä, eikä muuttumaton tarina tyhjennä merkintää. `narration` tallentaa kuitattujen ajankulkukorttien määrän/vaiheet, edellisen kortin luonteen ja seuraavassa kohtauksessa esitettävät lyhyet tulokset. Ne toistetaan latauksessa samasta toimintahistoriasta.
...
Tarina ei itsessään lisää kuukausia. Näkymän valmistelu käsittelee tarvittaessa automaattisesti seuraavat hiljaiset odotukset niiden oikean työkalenterin kautta. Näkyviä odotuskortteja on enintään kolme koko peliin, enintään yksi vaiheessa, eikä kahta peräkkäin. Jokainen valmistumishetki, rinnakkainen työ, erääntyvä tulos ja määräaika käsitellään edelleen järjestyksessä. Tunnetut rutiinivahvistukset yhdistetään lyhyiksi huomioiksi seuraavaan kohtaukseen ja merkitään paljastetuiksi; lähteen koko teksti ja tuloshaara säilyvät. Muut tulokset pysäyttävät etenemisen. Vaihesiirtymä, merkittävä myönteinen tai kielteinen tulos, hyväksyminen ja lainvoima näkyvät erikseen. Vain kaikkien lupien viimeinen kuittaus yhdistyy samoja ehtoja tarkistavaan voittoruutuun. Jo luettua tarinaa ei paineta uudelleen seuraavan päätöksen päälle. Reaktiot valitaan täsmällisestä kortti- tai tuloshaara-ID:stä ilman satunnaisuutta tai tekstistä pääteltyjä vaikutuksia.
