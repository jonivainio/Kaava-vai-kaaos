...
**Julkinen julkaisu ja kirjautumaton koe läpäisty 9.9.2026 klo 02:08 Suomen aikaa.** Sites-versio 4 julkaistiin commitista `937bf50c69689eef2ad5eae4008ede29c6b2e744`. GitHubin `main` päivitettiin samaan toteutukseen. Osoite: https://kaava-vai-kaaos.joni-vainio.chatgpt.site/ . Käyttöoikeus oli ja säilyi `public`.

`tools/check-public-v5.mjs` avasi täysin uuden Chromium-kontekstin (alussa 0 evästettä), ilman ohitustokenia, tunnuksia, kirjautumista tai Authorization-headeria. HTTP 200, oikea v5-sisältö, ladatun JavaScriptin SHA-256 sama kuin paikallisessa buildissa. Koko kierros: 25 päätöstä, kolme vaihesiirtymää ja kolme uudelleenlatausta, 699 pisteen lupavoitto, ei selainvirheitä. Tulokset: `public-check.json`, kuvat `public/`, julkaisutunnisteet `release.json`. Julkisuutta ei päätelty pelkästä käyttöoikeusasetuksesta.

**Keskeneräiseksi jäävä laitevarmennus:** fyysinen puhelin ja edellä eritellyt muut selaimet. Toimeksiannon v5-toteutus, paikallinen hyväksyntä ja julkinen toimitus ovat valmiit. Julkaisun jälkeen tehdyt dokumentaatiocommitit tallentavat varmennustuloksen; ne eivät muuta julkaistua pelibundlea.

Myös käyttäjän jo avoinna ollut 007-välilehti päivitettiin näkyvän ”Päivitä tästä aloitusvalikosta” -toiminnon kautta. Uusi valikko näytti ”Vanha tai vioittunut tallennus on turvassa” ja ”Vie tallennus”. Uutta peliä ei aloitettu käyttäjän tallennuksen päälle. Tämä on lisäksi oikea vanhasta julkaistusta versiosta tehty päivityshavainto, ei fyysisen puhelimen koe.
